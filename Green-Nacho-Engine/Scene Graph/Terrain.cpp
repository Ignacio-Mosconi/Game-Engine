...
namespace gn
{
	...
	void Terrain::createHeightField(std::vector<std::vector<float>> heights, int rows, int columns, glm::vec3 scale)
	{
		_heights = heights;
		_heightmapRows = rows;
		...
		for (int row = 0; row < _heightmapRows; row++)
		{
			...
			{
				currentSample->height = (physx::PxI16)_heights[row][col];
		...
		physx::PxReal heightScale = (physx::PxReal)scale.y;
		...
		physx::PxHeightFieldGeometry hfGeometry(_heightField, physx::PxMeshGeometryFlags(), heightScale, rowScale, columnScale);

		_heightField = physicsManager->createHeighField(hfDesc);
		_rigidActor = physicsManager->createRigidActor(pxTransform, true);
		_material = physicsManager->createPhysicsMaterial(0.5f, 0.5f, 1.0f);
		_shape = physx::PxRigidActorExt::createExclusiveShape(*_rigidActor, hfGeometry, *_material);
